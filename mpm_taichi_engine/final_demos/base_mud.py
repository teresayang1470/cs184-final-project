...
mpm.add_cube(lower_corner=[0, 0], cube_size = [1, .3], material = MPMSolver.material_mud, color = 0x897D76, velocity=[0, -1])
...
for frame in range(210):
    ...
    if frame < 100 and frame % 10 == 0: 
        mpm.add_cube(lower_corner=[0.49, 0.8], cube_size = [0.025, 0.2], material = MPMSolver.material_mud, color = 0x897D76, velocity=[0, .01])
